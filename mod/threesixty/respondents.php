...
if (isset($user)) {
    ...
    if ($fromform = $mform->get_data()) {
        ...
        $myfromform = new stdClass();
        $myfromform = $fromform;
        $mailList = array();

        if(array_key_exists('sEmail',$fromform)){
            $mailList = $fromform->sEmail;
        }
        if($fromform->email != ''){
            $mailList[] = $fromform->email;
        }
        ...
        foreach($mailList as $resp_mail){
            $myfromform->email = $resp_mail;
...
function request_respondent($formfields, $analysisid, $senderfullname) {
    ...
    /*if(($formfields->email) == null){//Checks what field has data and sends accordingly
        $respondent->email = strtolower($formfields->sEmail);
    }else{
