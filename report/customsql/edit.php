...
if ($newreport = $mform->get_data()) {
    ...
    $recList = array();
    $userList = $DB->get_records('user',null,null,'id,username');
    foreach($newreport->emailto as $recId){
        foreach($userList as $user){
            if( $recId == $user->id ){
                $recList[] = $user->username;
            }
        }
    }
    $mynewreport = $newreport;
    $mynewreport->emailto = null;
    foreach($recList as $rec){
        $mynewreport->emailto = $mynewreport->emailto.$rec.',';
    }
    $mynewreport->emailto = rtrim($mynewreport->emailto, ',');

    if ($id) {
        $mynewreport->id = $id;
        $ok = $DB->update_record('report_customsql_queries', $mynewreport);
        if (!$ok) {
        ...
        $id = $DB->insert_record('report_customsql_queries', $mynewreport);
        if (!$id) {
