...
class report_customsql_edit_form extends moodleform {
    public function definition() {
        ...
        $capability = 'report/customsql:subscription';
        ...
        foreach($userList as $user){
            if(!$user->suspended && !$user->deleted){
                ...
                if (has_capability($capability, $context, $user)) {
                    $managerList[$user->id] = $user->lastname.' '.$user->firstname;
                }
            }
        }
        $mform->addElement('select', 'emailto', get_string('emailto', 'report_customsql'), $managerList);
        $mform->getElement('emailto')->setMultiple('true');
